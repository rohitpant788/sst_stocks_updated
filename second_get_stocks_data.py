...
import helper
from config import WORKBOOK_FOR_SST, WORKBOOK_MAIN_SHEET, WORKBOOK_TRACKING_SHEET, WORKBOOK_INVESTMENT_SHEET
...
def init():
    ...
    for i in range(1, row):
        ...
        spot = pd.DataFrame(helper.get_historical_data(sym, 7)).iloc[-1]
        ...
        # Price > 20MA
        moving_average_20 = last_row['20_day_ma']
        close_price = spot['Close']
        if close_price > moving_average_20:
            sh1.cell(i + 1, 10).value = "Yes"
        else:
            sh1.cell(i + 1, 10).value = "No"
